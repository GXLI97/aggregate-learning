...
for bag_size in [1000, 100, 10]:
  for opt in ['adam']:
    for lr in [0.01, 0.001, 0.0001, 0.00001]:
      line = 'python3 aggregate_mnist.py' + ' ' + str(bag_size) + \
              ' ' + opt + ' ' + str(lr) + ' ' + str(NUM_TRIALS) + \
              ' ' + str(LABEL_NOISE) + '\n'
...
file_out.write('#!/bin/bash\n')
sbatch_end = gen_sbatch_end(args.constraint, args.job_name)
file_out.write('#SBATCH --partition=' + 'gpu' + ' --cpus-per-task=1 --array=1-' + str(len(commands)) + sbatch_end + '\n')
file_out.write("eval \"$($HOME/mc3/bin/conda \'shell.bash\' \'hook\')\"" + '\n')
file_out.write(". ~/myenv/bin/activate" + '\n')
